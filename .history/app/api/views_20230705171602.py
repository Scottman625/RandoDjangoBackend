...
class getChatRoomViewSet(APIView):
    ...
    def post(self,request):
        ...
        condition = Q(user=current_user) & Q(user=User.objects.get(id=user_id))

        chatroom_list = ChatroomUserShip.objects.filter(condition).values_list('chatroom')

        chatroom = ChatRoom.objects.filter(id__in=chatroom_list)
