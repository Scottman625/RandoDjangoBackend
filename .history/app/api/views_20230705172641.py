...
class getChatRoomViewSet(APIView):
    ...
    def post(self,request):
        ...
        chatroom_list1 = set(ChatroomUserShip.objects.filter(user=current_user).values_list('chatroom'))
        chatroom_list2 = set(ChatroomUserShip.objects.filter(user=User.objects.get(id=user_id)).values_list('chatroom'))

        chatroom = chatroom_list1.intersection(chatroom_list2)
        print(chatroom(0))
