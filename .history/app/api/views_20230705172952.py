...
class getChatRoomViewSet(APIView):
    ...
    def post(self,request):
        ...
        chatroom_set2 = set(ChatroomUserShip.objects.filter(user=User.objects.get(id=user_id)).values_list('chatroom'))

        chatroom_set = chatroom_set1.intersection(chatroom_set2)
        print(list(chatroom_set[0]))

        chatroom = ChatRoom.objects.get(id=list(chatroom_set[0]))
