...
class ChatRoomViewSet(viewsets.GenericViewSet,
    ...
    def get_queryset(self):
        ...
        other_side_user_phone = self.request.query_params.get('other_side_user_phone')
        # if other_side_user_phone != '' and other_side_user_phone != None:
        #     print(other_side_user_phone)
        ...
        #     queryset = ChatRoom.objects.filter(id__in=common_chatroom_ids)
        # else:
    ...
    def create(self, request, *args, **kwargs):
        ...
        if ChatroomUserShip.objects.filter(user=user,chatroom=ChatRoom.objects.filter(id__in=common_chatroom_ids).first()).count() == 0:
            ...
            chatroomuserShip.save()
            serializer = serializers.UserSerializer(chatroom)
