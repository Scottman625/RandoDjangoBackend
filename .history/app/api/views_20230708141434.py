...
class MessageViewSet(APIView):
    ...
    def post(self, request):
        ...
        if user in chatroom_users:
            ...
            message.other_side_image_url = other_side_user.imageUrl
            message.other_side_phone = other_side_user.phone
            message.message_is_mine = True


            serializer = serializers.MessageSerializer(message)
            print('chatroom_name: ' + str(serializer.data['chatroom']))
            room_group_name = f"chat_{str(serializer.data['chatroom'])}"
            async_to_sync(channel_layer.group_send)(
                room_group_name,
                {
                    'type': 'chat_message',
                    'message': serializer.data['content'],
                    'other_side_image_url': serializer.data['other_side_image_url'],
                    'other_side_phone': serializer.data['other_side_phone'],
                    'create_at': serializer.data['create_at'],
                    'message_is_mine': serializer.data['message_is_mine'],
                    'should_show_time': serializer.data['should_show_time'],

                }
            )

            print(serializer.data['other_side_image_url'])
            print(serializer.data['other_side_phone'])
            print(serializer.data['create_at'])
            print(serializer.data['message_is_mine'])
            print(serializer.data['should_show_time'])
            return Response(serializer.data)
        else:
            return Response({'message': "have no authority"})