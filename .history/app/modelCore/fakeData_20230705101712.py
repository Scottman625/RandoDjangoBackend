...
def importMessage():
    ...
    chatroomMessage.chatroom = ChatRoom.objects.get(id=1)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first().user
    chatroomMessage.content = 'Hello!'
    chatroomMessage.save()

    chatroomMessage = ChatroomMessage()
    chatroomMessage.chatroom = ChatRoom.objects.get(id=1)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first()
    chatroomMessage.content = "What's your name?"
    chatroomMessage.save()

    chatroomMessage = ChatroomMessage()
    chatroomMessage.chatroom = ChatRoom.objects.get(id=2)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first()
    chatroomMessage.content = 'Hi!'
    chatroomMessage.save()

    chatroomMessage = ChatroomMessage()
    chatroomMessage.chatroom = ChatRoom.objects.get(id=3)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first()
    chatroomMessage.content = '嗨！'
    chatroomMessage.save()

    chatroomMessage = ChatroomMessage()
    chatroomMessage.chatroom = ChatRoom.objects.get(id=4)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first()
    chatroomMessage.content = '在幹嘛？'
    chatroomMessage.save()

    chatroomMessage = ChatroomMessage()
    chatroomMessage.chatroom = ChatRoom.objects.get(id=5)
    chatroomMessage.user = ChatroomUserShip.objects.filter(Chatroom=ChatRoom.objects.get(id=1)).first()
