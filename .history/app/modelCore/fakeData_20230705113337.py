...
def importMessage():
    ...
    condition2 = Q(user1=ChatroomUserShip.objects.filter(user=~condition1,chatroom=chatroom).first().user)
    condition3 = Q(user2=ChatroomUserShip.objects.filter(chatroom=chatroom).first().user)
    condition4 = Q(user2=ChatroomUserShip.objects.filter(user=~condition1,chatroom=chatroom).first().user)
